...
inception_resnet_v2(bs,71,71,192):'InceptionResnetV2/InceptionResnetV2/Conv2d_4a_3x3/Relu'/'InceptionResnetV2/InceptionResnetV2/Conv2d_4a_3x3/Conv2D'
...
tf.flags.DEFINE_string('input_dir', './dataset/Segment_and_resize40/', 'Input directory with images.')

tf.flags.DEFINE_string('output_dir', './adv/region_attack/tmp/', 'Output directory with images.')
...
tf.flags.DEFINE_integer('image_resize', 330, 'size of each diverse images.')
                    ...
                    if i == 0 and 'RPA' in FLAGS.attack_method:
                        ...
                        for l in range(int(FLAGS.ens)):
                            if l % 1 == 0:
                                ...
                                mask = patch_by_strides((batch_shape[0], batch_shape[1], batch_shape[2],
                                                         batch_shape[3]), (3, 3), FLAGS.probb)
                            elif l % 3 == 2:
                                mask = patch_by_strides((batch_shape[0], batch_shape[1], batch_shape[2],
                                                         batch_shape[3]), (5, 5), FLAGS.probb)
                            else:
                                mask = patch_by_strides((batch_shape[0], batch_shape[1], batch_shape[2],
                                                         batch_shape[3]), (7, 7), FLAGS.probb)
