...
tf.flags.DEFINE_string('output_dir', './adv/NAA/tmp/', 'Output directory with images.')
...
def main(_):
    ...
    with tf.Graph().as_default():
        ...
        amplification_ph = tf.compat.v1.placeholder(dtype=tf.float32, shape=batch_shape)
                ...
                for i in range(num_iter):
                    if i==0:
                        ...
                        for l in range(int(FLAGS.ens)):
                            ...
                            w, feature,pres = sess.run([weights_tensor, opt_operations[0],pred],feed_dict={ori_input: images_tmp2, adv_input: images_tmp2, label_ph: labels})
